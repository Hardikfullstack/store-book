...
export default function DashboardClient({
    ...
    useEffect(() => {
        ...
        const shouldShowSetup =
            globalThis.sessionStorage.getItem("storebook_setup_pending") ===
            "1";
    ...
    useEffect(() => {
        ...
        const updateStats = () => {
            ...
            const filteredSales = rawSales.filter(
                (s) =>
                    (s.timestamp || s.updated_at) * 1000 >= cutoff &&
                    !s.isDeleted,
            );
            const filteredUdhaar = rawUdhaars.filter(
                (u) =>
                    (u.timestamp || u.updated_at) * 1000 >= cutoff &&
                    !u.isDeleted,
            );
            const filteredExpenses = rawExpenses.filter(
                (e) =>
                    (e.timestamp || e.updated_at) * 1000 >= cutoff &&
                    !e.isDeleted,
            );
            ...
            const totalSales = filteredSales.reduce(
                (acc, doc) => acc + (doc.total_amount || 0),
                0,
            );
            const totalUdhaar = filteredUdhaar.reduce(
    ...
    }, [isPremium, storeId, canAccessCost]);
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
                        Real-time sync via Data Connect Delta.
                    </p>
                </div>
                <div>
                    <select
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((stat, i) => (
                    <div
                        key={i}
                        className="glass-card p-6 flex flex-col justify-between hover:shadow-md transition-shadow duration-300"
                    >
                        <div className="flex justify-between items-start mb-4">
                            <div
                                className={`p-3 rounded-xl shadow-sm ${stat.color}`}
                            >
                                {stat.icon}
                            </div>
                            <div
                                className={`flex items-center space-x-1 text-sm font-medium px-2 py-1 rounded-full ${stat.trendUp ? "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400" : "bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400"}`}
                            >
                                {stat.trendUp ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
                <div className="lg:col-span-2 glass-card p-6 h-80 flex items-center justify-center border-dashed dark:border-gray-800">
                    <DashboardCharts
                        ...
                        itemsData={stats.itemsData}
                        saleItemsData={stats.saleItemsData}
                    />
                </div>
