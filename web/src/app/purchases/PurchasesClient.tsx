...
import { Search, ChevronDown, ChevronRight, Loader2, ShoppingBag } from 'lucide-react';
import { dataConnect } from '@/lib/firebase';
import { executeQuery } from 'firebase/data-connect';
import { listPurchasesRef, getPurchasesCountRef, getPurchaseDetailsRef, OrderDirection } from '@/dataconnect';
...
export default function PurchasesClient({
  ...
  useEffect(() => {
    ...
    const fetchTotal = async () => {
      try {
        const resp = await executeQuery(getPurchasesCountRef(dataConnect, { storeId }));
  ...
  useEffect(() => {
    ...
    const fetchPurchases = async () => {
      setIsLoading(true);
      try {
        const resp = await executeQuery(listPurchasesRef(dataConnect, {
          storeId,
          limit: pageSize,
  ...
  }, [storeId, currentPage, sortField, sortDirection]);
  ...
  const filteredPurchases = purchases.filter(p => {
    ...
    if (minAmountFilter && p.totalAmount < parseFloat(minAmountFilter)) return false;
    if (maxAmountFilter && p.totalAmount > parseFloat(maxAmountFilter)) return false;
    return true;
  });
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center w-full">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1">
                <span className="text-xs text-gray-500 dark:text-gray-400 mr-1">Amount Range:</span>
                <input
                  type="number"
                  placeholder="Min"
                ...
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                      <button type="button" onClick={() => handleSort('timestamp')} className="inline-flex items-center gap-1 hover:text-gray-700 dark:hover:text-gray-200">
                        Date {sortField === 'timestamp' ? (sortDirection === 'asc' ? '▲' : '▼') : '↕'}
                      </button>
                    </th>
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                      <button type="button" onClick={() => handleSort('supplier_name')} className="inline-flex items-center gap-1 hover:text-gray-700 dark:hover:text-gray-200">
                    ...
                    <th className="px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-right">
                      <button type="button" onClick={() => handleSort('total_amount')} className="inline-flex items-center justify-end gap-1 w-full hover:text-gray-700 dark:hover:text-gray-200">
                          ...
                          <div className="flex flex-col gap-1">
                            {items.map((item: any) => (
                              <div key={item.id} className="text-sm text-gray-700 dark:text-gray-300">
                                {item.itemName} <span className="text-gray-400 dark:text-gray-500 text-xs">({item.quantity} {item.unit})</span>
                              </div>
                            ))}
                          </div>
                        </td>
