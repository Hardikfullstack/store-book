...
export default function ItemsClient({
    ...
    const lastSynced = useSelector(
        (state: RootState) => state.inventory.lastSynced,
    );
        ...
        const fetchItems = async () => {
            ...
            try {
                ...
                const response = await executeQuery(
                    getActiveItemsRef(dataConnect, vars),
                    options,
                );
    ...
    const [hsnSuggestedRate, setHsnSuggestedRate] = useState<number | null>(
        null,
    );
    ...
    const handleDelete = async (id: string) => {
        if (confirm("Are you sure you want to delete this item?")) {
            try {
                await softDeleteItem(dataConnect, {
                    id,
                    updatedAt: Date.now(),
                });
                invalidateAllPages();
        ...
        <div className="space-y-6">
            ...
            <RestockQuantity
                ...
                onConfirm={async (payload) => {
                    ...
                    const nextQuantity =
                        Number(reStockQuantity.quantity || 0) +
                        payload.quantity;
                    const updatedItem = {
                        ...reStockQuantity,
                        ...
                        ...(canAccessCost && {
                            buy_price:
                                payload.buyPrice ??
                                reStockQuantity.buy_price ??
                                0,
                        }),
                        batchLotNumber:
                            payload.batchNumber ||
                            reStockQuantity.batchLotNumber,
                        expiryDate:
                            payload.expiryDate || reStockQuantity.expiryDate,
                        updated_at: Date.now(),
                    };
                    ...
                    const purchaseId = crypto.randomUUID();
                    const totalAmount =
                        payload.quantity * (payload.buyPrice || 0);
                    const taxAmount =
                        totalAmount * ((reStockQuantity.taxRate || 0) / 100);
