...
export async function archiveOldData(
    ...
    await assertPermission("canAccessAdmin");
...
export async function purgeStoreData(
    storeId: string,
...
export async function createStaffAccount(
    ...
    if (!session.storeId) {
        throw new Error(
            "No store context — cannot create staff without a store",
        );
    }

    const role = options?.role ?? "cashier";

    const virtualEmail = `${sanitizedUsername}@storebook.internal`;
          ...
          user_upsert(data: { id: $id, role: $role, createdAt: $createdAt, storeId: $storeId, canViewProfit: $canViewProfit, canDelete: $canDelete })
        }`,
            {
                variables: {
                    ...
                    canViewProfit: options?.canViewProfit ?? false,
...
export async function updateStaffRole(
    ...
    try {
        ...
        if (
            session.role === "owner" &&
            (target.ownerId !== session.uid ||
                target.storeId !== session.storeId)
        ) {
            return {
                success: false,
                error: "Unauthorized — cannot modify this staff member",
            };
        }

        await dc.executeGraphql(
            `mutation UpdateUserRole($id: String!, $role: String!) { user_update(id: $id, data: { role: $role }) }`,
            { variables: { id: targetUid, role: newRole } },
        );
        ...
        try {
            ...
            console.warn(
                "Could not revoke refresh tokens for user",
                targetUid,
                e,
            );
        }
...
export async function deleteStaffAccount(
    ...
    try {
        ...
        if (
            ...
            return {
                success: false,
                error: "Unauthorized — cannot delete this staff member",
            };
        }
...
export async function getStaffByStore(
    ...
    try {
        ...
        const response = await dc.executeGraphql(
          ...
          users(where: { storeId: { eq: $storeId } }) {
            id
            username
            role
            createdAt
...
export async function getSalesTrendData(
    ...
    success: boolean;
    data?: { date: string; totalValue: number; transactionCount: number }[];
    error?: string;
}> {
    const session = await getSession();
    if (!session) return { success: false, error: "Unauthorized" };
    ...
    if (
        ...
        !allowedStores.includes(storeId as string)
    ) {
        return {
            success: false,
    ...
    try {
        ...
        const data = Array.from(dailyMap.entries())
            ...
            .map(([date, vals]) => ({
                date,
                totalValue:
                    Math.round((vals.totalValue + Number.EPSILON) * 100) / 100,
    ...
    try {
        ...
        for (const si of items) {
            ...
            if (liveItem.quantity < neededQty) {
                ...
                return {
                    success: false,
                    error: `Insufficient stock for '${liveItem.name}': have ${liveItem.quantity}, need ${neededQty}`,
                };
            }
 ...
 */
export async function getUdhaarCustomerBalances(storeId: string): Promise<{
    success: boolean;
    data?: {
        customerName: string;
        netBalance: number;
    ...
    try {
        ...
        const customerMap = new Map<
            string,
            { creditSum: number; paymentSum: number; lastTime: number }
        >();
        ...
        const data = Array.from(customerMap.entries()).map(
            ([customerName, vals]) => ({
                customerName,
                netBalance:
                    Math.round(
