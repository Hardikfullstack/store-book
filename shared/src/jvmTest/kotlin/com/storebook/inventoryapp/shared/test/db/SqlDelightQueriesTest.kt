...
class SqlDelightQueriesTest {
    ...
    fun `insertItem persists and retrieves correctly`() {
        ...
        database.storeBookQueries.insertItem("Chai Tea Bags", 50.0, "Box", 120.0, 180.0, 5.0, "Beverage", null, null, 5.0, now)
    ...
    fun `softDelete sets is_deleted=True — row exists but excluded from active queries`() {
        database.storeBookQueries.insertItem("Tea Powder", 20.0, "Pack", 80.0, 120.0, 3.0, "Beverage", null, null, 5.0, 1721000000000L)
    ...
    fun `updateItem clears is_synced flag so SyncWorker picks up change`() {
        database.storeBookQueries.insertItem("Coffee Pack", 30.0, "Box", 250.0, 400.0, 5.0, "Beverage", null, null, 18.0, 1721000000000L)
        database.storeBookQueries.updateItem(name = "Coffee Pack Upgraded", quantity = 35.0, unit = "Box", buy_price = 260.0, sell_price = 420.0, low_stock_threshold = 5.0, category = "Beverage", photo_path = null, hsn_code = null, tax_rate = 18.0, updated_at = 1721000001000L, id = 1L)
    ...
    fun `transaction rollback on partial failure — all writes inside block are undone`() {
        ...
        database.storeBookQueries.insertItem("Sugar Pack A", 15.0, "Kg", 40.0, 60.0, 3.0, "Spice", null, null, 5.0, 1721000000000L)

        // Wrap transaction: modify TWO items; second write throws → both revert
        var threw = false
        try {
            database.transaction {
                database.storeBookQueries.updateItem(name = "Sugar Modified", quantity = 16.0, unit = "Kg", buy_price = 45.0, sell_price = 65.0, low_stock_threshold = 3.0, category = "Spice", photo_path = null, hsn_code = null, tax_rate = 5.0, updated_at = 1721000001000L, id = 1L)
                ...
                database.storeBookQueries.updateItem(name = "Poison", quantity = 0.0, unit = "", buy_price = 0.0, sell_price = 0.0, low_stock_threshold = 0.0, category = "X", photo_path = null, hsn_code = null, tax_rate = 0.0, updated_at = 0L, id = 99999L)
    ...
    fun `integer overflow safety — large monetary amounts stored as REAL do not wrap`() {
        ...
        database.storeBookQueries.insertItem("Gold Bullion", hugeAmount, "Kg", 5000000.0, 6000000.0, 1.0, "Luxury", null, null, 3.0, 1721000000000L)
