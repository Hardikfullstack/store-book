...
class FirestoreSyncManager(
    ...
    suspend fun syncAllData(onProgress: (Int, String) -> Unit = { _, _ -> }) =
        withContext(Dispatchers.IO) {
            ...
            val userId = user.phoneNumber ?: user.uid // Web uses phone number as user id if available
        ...
        cursor.use { c ->
            while (c.moveToNext()) {
                ...
                if (cloudId == null || cloudId.isBlank()) {
                    ...
                    val localUpdatedAt = c.getLong(c.getColumnIndexOrThrow(StoreBookDbHelper.KEY_UPDATED_AT))
                    val remoteDoc = remoteDocs.documents.find { it.getString("cloud_id") == cloudId || it.id == cloudId }
                    if (remoteDoc != null) {
                        val remoteUpdatedAt = remoteDoc.getLong("updated_at") ?: 0L
                        if (remoteUpdatedAt > localUpdatedAt) {
                            // Web/Cloud has a newer version. Do not push. PULL phase will overwrite local.
                            continue
                        }
                    }
                }
                ...
                dataMap["cloud_updated_at"] = com.google.firebase.firestore.FieldValue.serverTimestamp()
    ...
    fun startRealtimeSync(storeId: String) {
        ...
        val tables = listOf(
            StoreBookDbHelper.TABLE_ITEMS to listOf(
                StoreBookDbHelper.KEY_ITEM_NAME,
                StoreBookDbHelper.KEY_ITEM_QTY,
                StoreBookDbHelper.KEY_ITEM_UNIT,
                StoreBookDbHelper.KEY_ITEM_BUY_PRICE,
            ...
            StoreBookDbHelper.TABLE_SALES to listOf(
                StoreBookDbHelper.KEY_TIMESTAMP,
                StoreBookDbHelper.KEY_SALE_TOTAL,
                StoreBookDbHelper.KEY_SALE_DISCOUNT,
                StoreBookDbHelper.KEY_SALE_CUSTOMER,
            ...
            StoreBookDbHelper.TABLE_SALE_ITEMS to listOf(
                StoreBookDbHelper.KEY_SI_SALE_ID,
                StoreBookDbHelper.KEY_SI_ITEM_ID,
                StoreBookDbHelper.KEY_SI_ITEM_NAME,
                StoreBookDbHelper.KEY_SI_UNIT,
            ...
            StoreBookDbHelper.TABLE_UDHAAR to listOf(
                StoreBookDbHelper.KEY_UDHAAR_CUSTOMER,
                StoreBookDbHelper.KEY_UDHAAR_AMOUNT,
                StoreBookDbHelper.KEY_UDHAAR_TYPE,
                StoreBookDbHelper.KEY_TIMESTAMP,
            ...
            StoreBookDbHelper.TABLE_EXPENSES to listOf(
                StoreBookDbHelper.KEY_EXPENSE_TYPE,
                StoreBookDbHelper.KEY_EXPENSE_DESC,
                StoreBookDbHelper.KEY_EXPENSE_AMOUNT,
                StoreBookDbHelper.KEY_TIMESTAMP,
            ...
            StoreBookDbHelper.TABLE_SUPPLIERS to listOf(
                "name",
                "phone",
                "gstin",
                "address",
            ),
            StoreBookDbHelper.TABLE_PURCHASES to listOf(
                "supplier_id",
                "supplier_name",
                "total_amount",
                "tax_amount",
            ...
            StoreBookDbHelper.TABLE_PURCHASE_ITEMS to listOf(
                "purchase_id",
                "item_id",
                "item_name",
                "quantity",
