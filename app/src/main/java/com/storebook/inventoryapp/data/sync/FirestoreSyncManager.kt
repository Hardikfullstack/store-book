...
class FirestoreSyncManager(context: Context) {
    ...
    suspend fun syncAllData() = withContext(Dispatchers.IO) {
        ...
        try {
            // Sync Items Table
            syncTable(userId, StoreBookDbHelper.TABLE_ITEMS, listOf(
                StoreBookDbHelper.KEY_ITEM_NAME, StoreBookDbHelper.KEY_ITEM_QTY,
                StoreBookDbHelper.KEY_ITEM_UNIT, StoreBookDbHelper.KEY_ITEM_BUY_PRICE,
                StoreBookDbHelper.KEY_ITEM_SELL_PRICE, StoreBookDbHelper.KEY_ITEM_CATEGORY
            ))

            // Sync Sales Table
            syncTable(userId, StoreBookDbHelper.TABLE_SALES, listOf(
                StoreBookDbHelper.KEY_TIMESTAMP, StoreBookDbHelper.KEY_SALE_TOTAL,
                StoreBookDbHelper.KEY_SALE_CUSTOMER, StoreBookDbHelper.KEY_NOTES
            ))

            // Sync Udhaar Table
            syncTable(userId, StoreBookDbHelper.TABLE_UDHAAR, listOf(
                StoreBookDbHelper.KEY_UDHAAR_CUSTOMER, StoreBookDbHelper.KEY_UDHAAR_AMOUNT,
                StoreBookDbHelper.KEY_UDHAAR_TYPE, StoreBookDbHelper.KEY_TIMESTAMP
            ))
    ...
    private suspend fun syncTable(userId: String, tableName: String, columnsToMap: List<String>) {
        ...
        cursor.use { c ->
            while (c.moveToNext()) {
                ...
                firestore.collection("users").document(userId)
                    .collection(tableName).document(cloudId)
        ...
        val remoteDocs = firestore.collection("users").document(userId)
