...
class PurchaseViewModel(
    ...
    suspend fun getPurchaseItems(purchaseId: Long): List<PurchaseItemDetail> {
        return _purchases.value.find { it.id == purchaseId }?.items ?: emptyList()
    }
    ...
    fun addPurchase(
        ...
        viewModelScope.launch {
            val pId = purchaseRepository.insertPurchase(
            ...
            itemsInCart.forEach { item ->
                purchaseRepository.insertPurchaseItem(pId, item.itemId, item.itemName, item.quantity, item.unit, item.buyPrice)
