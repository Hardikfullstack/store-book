...
fun InvoicePdfPreviewScreen(
    ...
    fun getOrGeneratePdfFile(): File? {
        val file = File(context.cacheDir, "Invoice_$saleId.pdf")
        if (file.exists()) {
            file.delete()
        }
    ...
    fun sharePdf() {
        ...
        if (pdfFile != null) {
            ...
            android.widget.Toast
                .makeText(context, "Cannot share — no items in invoice", android.widget.Toast.LENGTH_SHORT)
                .show()
        }
    }
    ...
    fun sharePdfWhatsApp() {
        val pdfFile = getOrGeneratePdfFile()
        if (pdfFile != null) {
            val msg = "Invoice #$saleId - Total: ${totalAmount.toRupee()}"
            ShareUtils.sharePdfWhatsApp(context, pdfFile, null, msg)
        } else {
            android.widget.Toast
                .makeText(context, "Cannot share — no items in invoice", android.widget.Toast.LENGTH_SHORT)
                .show()
    ...
    fun handleSave() {
        val pdfFile = getOrGeneratePdfFile()
        if (pdfFile != null) {
            saveLauncher.launch("Invoice_$saleId.pdf")
        } else {
            android.widget.Toast
                .makeText(context, "Cannot save — no items in invoice", android.widget.Toast.LENGTH_SHORT)
                .show()
        }
    }
    ...
    fun openPdf() {
        ...
        if (pdfFile != null) {
            ...
            android.widget.Toast
                .makeText(context, "Cannot open — no items in invoice", android.widget.Toast.LENGTH_SHORT)
                .show()
        }
    }
